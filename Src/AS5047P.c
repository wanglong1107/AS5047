...
#include "spi.h"
#include "main.h"
#define AS5047P_CS_1  		HAL_GPIO_WritePin(nCS_GPIO_Port, nCS_Pin, GPIO_PIN_SET)//;delay_ms(1);  //PA4
#define AS5047P_CS_0  		HAL_GPIO_WritePin(nCS_GPIO_Port, nCS_Pin, GPIO_PIN_RESET)//;delay_ms(1);   //PA4
...
unsigned int AS5047P_Read(unsigned int address)//读取带地址的数据
{
	...
	uint8_t TxData[2];
	uint8_t RxData[2];
	TxData[0]=(uint8_t)((address&0xff00)>>8);
	TxData[1]=(uint8_t)(address&0x00ff);
...
//	address = address | AS5047P_RD; // it's a read command
	...
	AS5047P_CS_1;
//	data = (((uint16_t)RxData[0])<<8) & 0x3F00 + RxData[1];  // filter bits outside data, strip bit 14..15
...
unsigned int AS5047P_ReadData(void)//读取SPI数据
{
	...
	AS5047P_CS_0;
//	data=SPI1_ReadWriteByte(0xFFFF);//设定发送0xFFFF，去掉I/O翻转时间
//	HAL_SPI_Transmit(&hspi2, (uint8_t *)(&address), 1, 1000);
	HAL_SPI_TransmitReceive(&hspi2, (uint8_t *)(&address), (uint8_t *)(&data), 1, 1000);
	AS5047P_CS_1;
//	data = (((uint16_t)RxData[0])<<8) & 0x3F00 + RxData[1];
	data = data & 0x3FFF;  // filter bits outside data, strip bit 14..15
	return data;
}
//////////////////////////////////////////////////////
